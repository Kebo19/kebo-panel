...
import {
  ...
  AlertTriangle, FileText, Wallet, TrendingDown, ArrowLeft, Phone, Hash
} from "lucide-react";

interface Cari {
  id: string; cari_kodu: string; unvan: string; vergi_no: string;
  vergi_dairesi: string; telefon: string; adres: string; tip: string;
}
...
export default function CarilerPage() {
  ...
  const [form, setForm] = useState({ cari_kodu: "", unvan: "", vergi_no: "", vergi_dairesi: "", telefon: "", adres: "", tip: "tedarikci" });
  ...
  // Cari bazlı fatura istatistikleri
  const cariStats = useMemo(() => {
    const map = new Map<string, { toplam: number; bekliyor: number; faturaSayisi: number }>();
    cariler.forEach(c => map.set(c.id, { toplam: 0, bekliyor: 0, faturaSayisi: 0 }));
    return map;
  }, [cariler]);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {filtreliCariler.map(c => (
                <div key={c.id} onClick={() => cariDetayAc(c)}
                  ...
                  <div className="mt-4 pt-3 border-t border-[#1a2236] flex items-center justify-between">
                    <span className="text-[10px] text-gray-600">Faturalar için tıklayın</span>
                    <FileText size={12} className="text-gray-700 group-hover:text-blue-400 transition-colors" />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input value={form.vergi_dairesi} onChange={e => setForm({ ...form, vergi_dairesi: e.target.value })} placeholder="..." className={inputCls} />
                </div>
              </div>
              <div>
