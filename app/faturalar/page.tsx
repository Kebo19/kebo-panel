...
import {
  ...
  AlertTriangle, Building2, Upload, RefreshCw, Calendar, ChevronDown, ChevronUp
} from "lucide-react";
    ...
    <table className="w-full text-xs">
      ...
      <tbody className="divide-y divide-[#0f1624]">
        {liste.map(f => (
          <tr key={f.id} className="hover:bg-white/[0.02] transition-colors">
            <td className="px-4 py-3 text-white font-medium max-w-[180px] truncate">{f.cari_unvan}</td>
      ...
      <tfoot>
        <tr className="border-t border-[#1a2236] bg-[#080b14]">
          <td colSpan={3} className="px-4 py-3 text-[10px] text-gray-600 font-bold uppercase">{liste.length} fatura</td>
          ...
          <div className="flex flex-wrap gap-2 items-center justify-between">
            <div className="flex gap-1 bg-[#080b14] border border-[#1a2236] rounded-xl p-1">
              {[
                ...
                { key: "tarih", label: "Tarih Aralığı" },
                { key: "cari", label: "Cari Bazlı" },
              ].map(m => (
              ...
              <div className="space-y-3">
                {cariGruplari.map(g => (
                  <div key={g.unvan} className="bg-[#0c0f1a] border border-[#1a2236] rounded-2xl overflow-hidden">
                    ...
                    {cariFiltre === g.unvan && (
                      <div className="border-t border-[#1a2236] overflow-x-auto">
                        <FaturaTablosu liste={g.faturalar} />
                      </div>
                    )}
                  </div>
                ))}
